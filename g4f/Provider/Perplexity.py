    ...
    async def create_async_generator(
        ...
        **kwargs,
        ...
        async with StreamSession(
            ...
            is_followup = (
                hasattr(conversation, "last_backend_uuid")
                and conversation.last_backend_uuid
            )

            # debug.log(f"Perplexity: is_followup={is_followup}")
            if is_followup:
                debug.log(
                    f"Perplexity: followup with last_backend_uuid={conversation.last_backend_uuid}, read_write_token={getattr(conversation, 'read_write_token', None)}"
                )
            ...
            if not is_followup:
                ...
                data = {
                    "params": {
                        "last_backend_uuid": getattr(
                            conversation, "last_backend_uuid", None
                        ),
                        "read_write_token": getattr(
                            conversation, "read_write_token", None
                        ),
                        "attachments": [],
                        "language": "en-US",
                        "timezone": "America/Los_Angeles",
            ...
            async with session.post(QUERY_ENDPOINT, json=data) as response:
                ...
                async for json_data in sse_stream(response):
                    ...
                    if "read_write_token" in json_data and not hasattr(
                        conversation, "read_write_token"
                    ):
                    ...
                    if "thread_url_slug" in json_data and (
                        not hasattr(conversation, "thread_url_slug")
                        or not conversation.thread_url_slug
                    ):
                    ...
                    for block in json_data.get("blocks", []):
                        ...
                        if block.get("intended_usage") == "sources_answer_mode":
                            sources = block.get("sources_mode_block", {}).get(
                                "web_results", []
                            )
                            continue
                                ...
                                "".join(
                                    [
                                        ...
                                        for chunk in [
                                            ...
                                            else YouTubeResponse(
                                                item.get("url").split("=").pop()
                                            )
